...
const clientId = '10440942302-06u6d55j9acpe88417kqk0em2shc2ad2.apps.googleusercontent.com'
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const user = await User.findOne({ ds_email: params.ds_email, ic_emailverified: true }).select("+ds_password");
      // const user = await User.findOne({ ds_email: params.ds_email }).select("+ds_password");
      
      // Check if the provided password is correct
      if (!(await user.correctPassword(params.ds_password, user.ds_password)) &&
      ...
      if (user.accountcanceled === true) {
        return sendError(req, res, 401, "Your account has been canceled.");
      }
  ...
  try {
    try {
      ...
      const payload = ticket.getPayload();
        ...
        {
          if (
            !(await user.correctPassword(params.ds_password, user.ds_password))
          ) {
...
exports.setpersonaldata = async(req, res) => {
  ...
  try {
    ...
    if(personalData.ic_profile == 0){
      ...
      user.ds_avatar = req.files[0] === undefined ? user.ds_avatar : req.files[0].filename;
    }
...
exports.setaddress = async(req, res) => {
  ...
  try {
    let user = await User.findOneAndUpdate({ds_email: address.ds_email}, address);
    res.status(200).json({
      status: "success",
      user
    })
...
exports.setnewpassword = async (req, res) => {
  ...
  try {
    ...
    try {
      const user = await User.findOne({ ds_email: params.ds_email, ic_emailverified:true}).select(
        "+ds_password"
      );
      if (
        !(await user.correctPassword(params.oldpassword, user.ds_password))
      ) {
        return sendError(req, res, 401, "Senha incorreta");
      }
...
exports.setaccountcanceled = async (req, res) => {
  ...
  try {
    ...
    user.account_cancellation.id_accountcancellationtype = params.id_accountcancellationtype;
    user.account_cancellation.ds_accountcancelcomment = params.ds_accountcancelcomment;
    await user.save();
    res.status(200).json({
      status: "success",
    })
...
exports.getmyinfo = async (req, res) => {
  try {
    let user = await User.findOne({ic_profile: params.ic_profile, ds_email: params.ds_email});
    res.status(200).json({
      status: "success",
      user
    })
...
exports.getaccountcancelreason = async (req, res) => {
  ...
  try {
      const reasonList = await AccountCancellationTypes.find({nm_sitelanguages:language});
      res.status(200).json({
          status: "success",
          reasonList,
      });
...
exports.getnotifications = async (req, res) => {
  ...
  try {
    let notifications = await Notification.find({ id_customer : mongoose.Types.ObjectId(req.authinfo.id) }).sort({createdAt: -1});
...
exports.setViewedNotification = async (req, res) => {
  ...
  try{
    ...
    noti.ic_viewed = true;
    await noti.save();
    let notifications = await Notification.find({ id_customer : mongoose.Types.ObjectId(params.id_customer) }).sort({createdAt: -1});
    return res.status(200).json({
      status: "success",
      notifications,
    });
...
exports.contactus = async (req, res) => {
  const { language } = req.query;
  try {
    const reasonListClient = await ContactReason.find({nm_sitelanguages:language, ic_profile:false});
    const reasonListResearcher = await ContactReason.find({nm_sitelanguages:language, ic_profile:true});
...
exports.addcontact = async (req, res) => {
  const {tp_customer, id_contactreason, ds_contactcomments, cd_order, nm_customer, ds_email, ic_profile, id_user} = req.body;
  try {
    const attachedFiles = req.files.map((item) => { return item.filename});
    await ContactUs.create({ds_telephone:tp_customer, id_contactreason, ds_contactcomments, cd_order, nm_customer, ds_email, tp_customer:ic_profile, attachedFiles, id_user})
    res.status(200).json({
      status: "success",
    })
  } catch (err) {
...
exports.getlanguagelist = async (req, res) => {
  const { language } = req.query;
  try {
      const languagelist = await Language.find({nm_sitelanguages:language});
      res.status(200).json({
          status: "success",
          languagelist,
      });
...
exports.getlanguageproficiency = async (req, res) => {
  const { language } = req.query;
  try {
      const proficiencylist = await LanguageProficiency.find({nm_sitelanguages:language});
      res.status(200).json({
          status: "success",
          proficiencylist,
      });
...
exports.getspecialitylist = async (req, res) => {
  const { language } = req.query;
  try {
      const specialitylist = await Speciality.find({nm_sitelanguages:language});
      res.status(200).json({
          status: "success",
          specialitylist,
      });
...
exports.getprofileofresearcher = async(req, res) => {
  ...
  try {
      let reviews = await Order.find({id_researcher: id_researcher, ic_review: true}).select('review')
      let profile = await User.findOne({_id: id_researcher});
      let proposalcnt = await Order.count({bids: mongoose.Types.ObjectId(id_researcher)});
